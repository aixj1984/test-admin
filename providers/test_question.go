...
type ITestQuestionProvider interface {
	...
	GetMore(array, object interface{}, query_key, status string, test_id string, start, length int) (int64, error)
	UpdateOne(m interface{}, cols ...string) (int64, error)
	Count(object interface{}, query_key, status string, test_id string) (int64, error)
...
func (p *TestQuestionProvider) GetMore(array interface{}, object interface{}, query_key, status string, test_id string, start, length int) (int64, error) {
	...
	if status == "2" {
		qb.Select("title",
			"question_lunjijichu.id",
			"question_type",
			"answer",
			"note",
			"IFNULL(test_question.question_id, 0) as status").
			From("question_lunjijichu").
			LeftJoin("(select * from course_test_question where test_id = ? ) as test_question").On("question_lunjijichu.id = test_question.question_id").
			Where(condition).
		...
		qb.Select("title",
			...
			"IFNULL(test_question.question_id, 0) as status").
			From("question_lunjijichu").
			InnerJoin("(select * from course_test_question where test_id = ? ) as test_question").On("question_lunjijichu.id = test_question.question_id").
			Where(condition).
			Limit(length).Offset(start)
	}
...
func (p *TestQuestionProvider) Count(object interface{}, query_key, status string, test_id string) (int64, error) {
	qb, _ := orm.NewQueryBuilder("mysql")
